"""
File name: N2O_emission_script_farm_1.py
Author: Ida Marie Blok Nielsen
Date: December 9, 2025
Course: RnD project

Description:
    This script is used to calculate N2O emission from danish cattle barns for (farm_1) by using
...
dates = ["20250829", "20250830", "20250831"]
valves = [1, 2, 3, 7, 8]
valve_labels = {1: "Background North", 2: "Deep Litter sampling", 3: "Barn sampling",
                7: "Background South", 8: "Background East"}
farm_name = "farm_1"
...
# input parameters
wind_file = "../data/DMI/farm_1_DMI.csv"
start_time = "2025-08-29 00:00:00"
stop_time = "2025-08-31 23:00:00"
print(wind_file)

# getting hourly averages and adding wind data

hourly_with_wind = add_wind_data(
    pvd, wind_file, start_time, stop_time)
...
# input parameters td=temperature data
...
# input parameter, farm_1 = two gases because of comparison between barn and Deep Litter
gas_column = ["ΔCO2_[mg/m3]", "ΔCO2_[mg/m3]_DL"]

# figure of gas production during time
figs = plot_single_gas_concentration(table_with_conc_mg_m3, gas_column)
plt.show()
# safe_column = gas_column.replace("/", "_")
figs.savefig(f"../figs/plots_with_conc/Barn_DL_CO2plot_{farm_name}.png")

# input parameters in [kg]
CO2_production = 2888
total_weight_animals = 134692
